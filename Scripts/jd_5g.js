...
function taskUrl(function_id,body) {
  ...
  return {
    ...
    headers: {
      ...
      "User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.3.8;14.4;ed61da6367d28105579c5c0df9e9472aa2d60c15;network/wifi")
    }
  }
}
...
function updateShareCodes(url = 'https://gitee.com/lxk0301/updateTeam/raw/master/jd_818.json') {
  return new Promise(resolve => {
    $.get({url,
      headers:{"User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.3.8;14.4;ed61da6367d28105579c5c0df9e9472aa2d60c15;network/wifi")}
    }, async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
